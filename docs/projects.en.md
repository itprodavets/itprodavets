...
- Description: Xnata is a multi-tenant SaaS platform for commerce and content storefronts where storefronts, back-office, AI access, and operations apps evolve as one product platform. The current workspace confirms a 41-project runtime portfolio: 29 backend services, 3 gateways, 3 UI surfaces, and 6 workers/tools. The platform covers storefront lifecycle, themes and content, catalog and merchandising, checkout/orders, customers, reviews, notifications, Telegram commerce, semantic search/RAG/MCP, logistics/delivery/warehouse/customs, printing, and staff operations. Live public storefronts today: [tea.community](https://tea.community) — a tea wiki; [tea.express](https://tea.express) — a China-local store shipping worldwide. Built by hand from the ground up and still growing — now developed largely through AI-assisted engineering.
  - **Platform core** — bounded-context services on .NET 10/C# 14 with shared platform libraries, gRPC contracts, PostgreSQL/Redis/RabbitMQ, outbox/inbox, Keycloak/OIDC, and observability through Serilog/OpenTelemetry/Prometheus/Grafana/Seq.
  - **Gateways & AI access** — AdminGateway, StorefrontGateway, and McpGateway. The MCP gateway translates AI-client requests into platform gRPC calls, uses API keys/scopes, and gives agents controlled access to catalogs, storefronts, orders, search, and semantic retrieval.
  - **Storefront runtime** — StorefrontService owns the Shopify-format theme aggregate, Live/Draft/Backup themes, Liquid rendering, domains/SSL, menus, homepage sections, blog posts, metaobjects, section/block definitions, app-block registration, card templates, ratings, and workspace access control.
  - **Themes-as-code and storefront editor** — a visual storefront editor in the admin panel plus a `dkh-theme` CLI for init/package/login/pull/push/dev/lint and guarded publish, so themes live in source control and deploy with a Shopify CLI-like workflow.
  - **Web/UI** — Admin Web UI on Next.js 16/React 19/TypeScript 5.9/Tailwind CSS 4/DaisyUI 5/FSD, typed API clients through Orval/Zod, TanStack Query, React Hook Form, Lexical rich text, Monaco editor, and SignalR; Storefront Web UI/PWA/Telegram Mini App on top of the shared storefront runtime.
  - **Data, search & AI** — ClickHouse analytics, Qdrant/vector search, EmbeddingWorker (Ollama primary, OpenAI fallback), SearchService/AssistantService, RAG/semantic retrieval, and recommendations context.
  - **Operations & fulfillment** — Cart/Order/Inventory/Logistics/Delivery/Warehouse/Customs, MediaService with S3/MinIO, Notification/Broadcast/Telegram services, API management, and a provider-agnostic PaymentService/plugin host.
  - **Staff apps & devices** — Avalonia desktop/Android Apps Suite for Warehouse/Inspector/POS, Keycloak OIDC PKCE, barcode workflows, ESC/POS receipts, ZPL labels, and a print-agent path for TCP/USB/BLE printers.
- Technologies: C# 14, .NET 10, ASP.NET Core, gRPC/Protocol Buffers, REST/BFF, GraphQL, MediatR, FluentValidation, EF Core, Dapper, PostgreSQL, MongoDB, Redis, RabbitMQ, ClickHouse, Qdrant, Ollama, OpenAI, Next.js 16, React 19, TypeScript 5.9, Tailwind CSS 4, DaisyUI 5, TanStack Query, React Hook Form, Zod, Orval, Lexical, Monaco Editor, Liquid, Shopify-style themes, `dkh-theme` CLI, Telegram Mini App SDK, Telegram.Bot, MTProto, Avalonia, ESC/POS, ZPL, Keycloak/OIDC/PKCE, S3/MinIO, Cloudflare Tunnel/Workers, Serilog, OpenTelemetry, Prometheus/Grafana/Seq, Docker, GitLab CI/CD, GitHub Actions, Playwright, Vitest, Jest, Testcontainers, DDD, CQRS, Outbox/Inbox, MCP (Model Context Protocol), Telegram
...
- Description: Tea-focused product line launched on Xnata. My co-founder brings the tea-industry domain depth; I build and run the product on the Xnata platform. Live public storefronts: [tea.community](https://tea.community) as a tea wiki / knowledge base and [tea.express](https://tea.express) as a China-local store shipping worldwide. Both storefronts run on one source-controlled Liquid bundle, `xnata-tea`: separate storefront profiles, catalogs, navigation, branding/settings, and journeys without theme forks. Inside: localized product and article catalog, tea research/archive, reviews, request-price-and-availability basket, customer account/wishlist, mobile/Telegram shell, SEO/structured data, custom domains, and storefront-specific visual systems.
- Technologies: C# 14, .NET 10, StorefrontService, StorefrontGateway, Liquid, `dkh-theme` CLI, EF Core, Dapper, MediatR, FluentValidation, PostgreSQL, Redis, RabbitMQ, gRPC, REST API, Keycloak/OIDC, Serilog, OpenTelemetry, Docker, Next.js 16, React 19, TypeScript 5.9, Tailwind CSS 4, DaisyUI 5, TanStack Query, React Hook Form, Zod, Orval, next-intl, Telegram Mini App SDK, PWA, SEO/structured data, DDD, xUnit, Playwright, GitLab CI/CD, GitHub Actions
