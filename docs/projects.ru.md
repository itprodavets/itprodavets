...
- Описание: Xnata — мультитенантная SaaS-платформа для commerce и content-витрин, где storefront, back-office, AI-доступ и операционные приложения развиваются как единая product platform. В текущем workspace подтверждён runtime-портфель из 41 проекта: 29 backend-сервисов, 3 gateway, 3 UI-поверхности и 6 workers/tools. Платформа покрывает жизненный цикл витрин, темы и контент, каталоги и мерчандайзинг, checkout/orders, клиентов, отзывы, уведомления, Telegram commerce, semantic search/RAG/MCP, logistics/delivery/warehouse/customs, печать и staff operations. Рабочие публичные витрины сейчас: [tea.community](https://tea.community) — чайная wiki; [tea.express](https://tea.express) — локальный магазин в Китае с отправкой по всему миру. Построил с нуля своими руками и продолжаю развивать — теперь во многом через AI-ассистированную разработку.
  - **Платформенное ядро** — bounded-context сервисы на .NET 10/C# 14 с общей platform library, gRPC-контрактами, PostgreSQL/Redis/RabbitMQ, outbox/inbox, Keycloak/OIDC и observability через Serilog/OpenTelemetry/Prometheus/Grafana/Seq.
  - **Gateways и AI-доступ** — AdminGateway, StorefrontGateway и McpGateway. MCP-гейтвей переводит запросы AI-клиентов в gRPC-вызовы платформы, использует API-key/scopes и даёт агентам контролируемый доступ к каталогам, витринам, заказам, поиску и semantic retrieval.
  - **Storefront runtime** — StorefrontService владеет Shopify-format theme aggregate, Live/Draft/Backup темами, Liquid-рендерингом, доменами/SSL, меню, homepage sections, blog posts, metaobjects, section/block definitions, app-block registration, card templates, рейтингами и workspace access control.
  - **Themes-as-code и редактор витрин** — визуальный storefront editor в админке плюс CLI `dkh-theme` для init/package/login/pull/push/dev/lint и guarded publish, чтобы темы жили в source control и деплоились по модели Shopify CLI.
  - **Web/UI** — Admin Web UI на Next.js 16/React 19/TypeScript 5.9/Tailwind CSS 4/DaisyUI 5/FSD, typed API clients через Orval/Zod, TanStack Query, React Hook Form, Lexical rich text, Monaco editor и SignalR; Storefront Web UI/PWA/Telegram Mini App поверх общего storefront runtime.
  - **Data, search и AI** — ClickHouse analytics, Qdrant/vector search, EmbeddingWorker (Ollama primary, OpenAI fallback), SearchService/AssistantService, RAG/semantic retrieval и recommendations context.
  - **Operations и fulfillment** — Cart/Order/Inventory/Logistics/Delivery/Warehouse/Customs, MediaService с S3/MinIO, Notification/Broadcast/Telegram services, API management и provider-agnostic PaymentService/plugin host.
  - **Staff apps и устройства** — Avalonia desktop/Android Apps Suite для Warehouse/Inspector/POS, Keycloak OIDC PKCE, barcode workflows, ESC/POS receipts, ZPL labels и print-agent path для TCP/USB/BLE принтеров.
- Технологии: C# 14, .NET 10, ASP.NET Core, gRPC/Protocol Buffers, REST/BFF, GraphQL, MediatR, FluentValidation, EF Core, Dapper, PostgreSQL, MongoDB, Redis, RabbitMQ, ClickHouse, Qdrant, Ollama, OpenAI, Next.js 16, React 19, TypeScript 5.9, Tailwind CSS 4, DaisyUI 5, TanStack Query, React Hook Form, Zod, Orval, Lexical, Monaco Editor, Liquid, Shopify-style themes, `dkh-theme` CLI, Telegram Mini App SDK, Telegram.Bot, MTProto, Avalonia, ESC/POS, ZPL, Keycloak/OIDC/PKCE, S3/MinIO, Cloudflare Tunnel/Workers, Serilog, OpenTelemetry, Prometheus/Grafana/Seq, Docker, GitLab CI/CD, GitHub Actions, Playwright, Vitest, Jest, Testcontainers, DDD, CQRS, Outbox/Inbox, MCP (Model Context Protocol), Telegram
...
- Описание: Чайное направление, запущенное на Xnata. Со-фаундер приносит доменную экспертизу по чаю; я строю и веду продукт на платформе Xnata. Рабочие публичные витрины: [tea.community](https://tea.community) как tea wiki / knowledge base и [tea.express](https://tea.express) как локальный магазин в Китае с отправкой по всему миру. Обе витрины работают поверх единого source-controlled Liquid bundle `xnata-tea`: разные storefront profiles, каталоги, навигация, branding/settings и сценарии без форков темы. Внутри: локализованный каталог товаров и статей, tea research/archive, reviews, request-price-and-availability basket, customer account/wishlist, mobile/Telegram shell, SEO/structured data, custom domains и storefront-specific визуальная система.
- Технологии: C# 14, .NET 10, StorefrontService, StorefrontGateway, Liquid, `dkh-theme` CLI, EF Core, Dapper, MediatR, FluentValidation, PostgreSQL, Redis, RabbitMQ, gRPC, REST API, Keycloak/OIDC, Serilog, OpenTelemetry, Docker, Next.js 16, React 19, TypeScript 5.9, Tailwind CSS 4, DaisyUI 5, TanStack Query, React Hook Form, Zod, Orval, next-intl, Telegram Mini App SDK, PWA, SEO/structured data, DDD, xUnit, Playwright, GitLab CI/CD, GitHub Actions
...
## PPC Entourage — оптимизация рекламы Amazon (Сентябрь 2021 – Декабрь 2021)

Краткосрочный контракт-подработка параллельно основной занятости — инженерия с фокусом на производительность для SaaS-платформы оптимизации Amazon PPC.

### PPC Entourage — платформа на Amazon Ads & Seller API
- Роль: Software Engineer (Contract)
- Продолжительность: Сентябрь 2021 – Декабрь 2021
- Описание: Платформа оптимизации рекламы Amazon для продавцов — автоматическое управление кампаниями и ставками, снижение ACoS, кастомная аналитика и возврат компенсаций FBA, чтобы продавцы масштабировали рекламу, не живя в таблицах. Работает поверх Amazon Advertising и Seller API: тянет данные по эффективности рекламы в больших объёмах и отправляет автоматические оптимизации обратно. Мой фокус — производительность: профилировал и оптимизировал продукт, чтобы он работал легче, что снизило стоимость инфраструктуры, и улучшил отзывчивость интерфейса — приложение стало ощутимо быстрее для клиентов.
...
- Роль: Full-Stack Developer
- Продолжительность: Август 2013 – Апрель 2015
